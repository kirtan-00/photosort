...
def make_video(dst, scenes=2, work=None):
    """A small h264 mp4 at dst: scenes=1 is 3 s of testsrc; scenes=2 is 2 s of testsrc followed by 2 s of
    plain blue (one hard cut at 2.0 s), built with the concat demuxer. Intermediates go in `work`
    ...
    run("-f", "lavfi", "-i", "testsrc=size=320x240:rate=10:duration=2,format=yuv420p", str(a))
    run("-f", "lavfi", "-i", "color=c=blue:size=320x240:rate=10:duration=2,format=yuv420p", str(b))
